...
<script>
  ...
  /**
   * @prop {string} workspaceId - Workspace ID for the selected item.
   */
  ...
  function handleSelect(event) {
    ...
    if (item.id === "none") {
      ...
      selectorTitle = capitalizeWords(item.title);
    }
  ...
  function handleCreate(event) {
    ...
    addItem(title.trim().toLowerCase(), '', 'task', null, workspaceId,);
  }
